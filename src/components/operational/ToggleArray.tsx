...
export const ToggleArray = ({ operationalMode, onModeChange }: ToggleArrayProps) => {
  ...
  const handleToggle = (key: keyof OperationalMode) => {
    const newValue = !operationalMode[key];
    console.log(`Toggle ${key}: ${newValue}`);
    onModeChange({ [key]: newValue });
        ...
        {toggles.map((toggle) => {
          ...
          return (
            <button
              ...
              className={`w-full p-3 sm:p-4 rounded-xl border transition-all duration-300 group relative overflow-hidden ${colorClasses.bg} ${colorClasses.border} ${colorClasses.glow} hover:scale-[1.02] active:scale-[0.98] touch-manipulation cursor-pointer`}
            >
              ...
              <div className="flex items-center space-x-3 relative z-10">
                ...
                <div className={`w-3 h-6 sm:h-8 rounded-full transition-all duration-300 ${isActive ? colorClasses.indicator : 'bg-gray-600/30'} ${isActive ? 'animate-pulse' : ''}`} />
              </div>
            </button>
          );
        })}
          ...
          <div className="flex justify-center space-x-3">
            {Object.entries(operationalMode).map(([key, active], index) => (
              <div
                ...
                onClick={() => handleToggle(key as keyof OperationalMode)}
              />
            ))}
          </div>

    ...
    <div className="bg-black/10 backdrop-blur-sm rounded-3xl border border-gray-700/20 p-4 sm:p-6 h-full relative overflow-hidden">
      ...
      <div className="absolute top-3 sm:top-4 left-3 sm:left-4 z-10 flex space-x-1 sm:space-x-2">
        {modes.map((modeOption) => (
          <button
            ...
            className={`px-2 sm:px-3 py-1 rounded-lg text-xs font-medium tracking-wider transition-all duration-500 touch-manipulation hover:scale-105 ${
              mode === modeOption.key
      ...
      <div className="absolute inset-4 sm:inset-6 flex items-center justify-center">
        ...
        {mode === 'cymatic' && (
          <div className={`relative ${isMobile ? 'w-48 h-48' : 'w-64 h-64'}`}>
            {/* Enhanced cymatic pattern rings */}
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div 
                className="w-6 h-6 bg-cyan-400/80 rounded-full"
                style={{
        ...
        {mode === 'spectral' && (
          <div className={`w-full ${isMobile ? 'h-32' : 'h-40'} flex items-end justify-center space-x-1`}>
            {/* Enhanced spectral bars */}
            {Array.from({ length: isMobile ? 32 : 48 }, (_, i) => (
              <div
                key={i}
                className="bg-gradient-to-t from-violet-600/60 via-violet-400/80 to-violet-200/90 rounded-t transition-all duration-150"
                style={{
        ...
        {mode === 'attractor' && (
          <div className={`relative ${isMobile ? 'w-48 h-48' : 'w-64 h-64'}`}>
            {/* Enhanced 3D attractor visualization */}
            <div className="absolute inset-0">
              {Array.from({ length: 18 }, (_, i) => (
                <div
                  key={i}
                  className="absolute w-3 h-3 bg-emerald-400/70 rounded-full shadow-lg shadow-emerald-400/30"
                  style={{
            ...
            {/* Enhanced attractor field lines */}
      ...
      <div className="absolute bottom-3 sm:bottom-4 left-3 sm:left-4 right-3 sm:right-4">
        <div className="flex justify-between items-center mb-2">
          <div className="text-xs text-gray-400/70 uppercase tracking-wider">
